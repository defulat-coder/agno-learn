# in_memory_storage_for_team.py — 实现原理分析
...
> 源文件：`cookbook/06_storage/in_memory/in_memory_storage_for_team.py`

## 概述

本示例展示 **InMemoryDb** 作为 **Team** 会话存储，并与 `postgres_for_team` / `dynamo_for_team` 同构：**两成员 Agent + Team 指令 + `output_schema=Article`**。

**核心配置一览：**

| 配置项 | 值 | 说明 |
|--------|------|------|
| `db` | `InMemoryDb(...)` | Team 级存储 |
