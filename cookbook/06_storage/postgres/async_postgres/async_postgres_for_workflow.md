# async_postgres_for_workflow.py — 实现原理分析
...
> 源文件：`cookbook/06_storage/postgres/async_postgres/async_postgres_for_workflow.py`

## 概述

本示例展示 **AsyncPostgresDb** 作为 **Workflow** 的 `db`：`Workflow(..., db=...)` 持久化工作流会话；结构为 **Team 研究步 + Agent 规划步**（与 `postgres_for_workflow.py` 一致）。
...
| `Workflow` | `db=AsyncPostgresDb(...)` | 会话表见源文件 |
