# postgres_for_agent.py — 实现原理分析

> 源文件：`cookbook/06_storage/postgres/postgres_for_agent.py`

## 概述

本示例展示 Agno 使用 **PostgresDb** 作为 **Agent** 会话/运行存储：`Agent(db=db, ...)` 将多轮对话与元数据写入该后端；与具体模型无关，重点在 **存储适配器** 与 `add_history_to_context` 行为。

**核心配置一览：**

| 配置项 | 值 | 说明 |
|--------|------|------|
| `db` | `PostgresDb(db_url=postgresql+psycopg://...)` | 连接串 |
...
`PostgresDb` 实现 `BaseDb` 契约；会话键由 `session_id` / `user_id`（若传入）决定。
...
未显式 `model` 时须依赖环境或会报错；生产环境应显式指定模型。
