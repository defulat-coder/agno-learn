# async_sqlite_for_agent.py — 实现原理分析

> 源文件：`cookbook/06_storage/sqlite/async_sqlite/async_sqlite_for_agent.py`

## 概述

本示例展示 Agno 使用 **AsyncSqliteDb** 作为 **Agent** 会话/运行存储：`Agent(db=db, ...)` 将多轮对话与元数据写入该后端；与具体模型无关，重点在 **存储适配器** 与 `add_history_to_context` 行为。
...
`AsyncSqliteDb` 实现 `BaseDb` 契约；会话键由 `session_id` / `user_id`（若传入）决定。
