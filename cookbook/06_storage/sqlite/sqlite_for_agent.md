...
> 源文件：`cookbook/06_storage/sqlite/sqlite_for_agent.py`

## 概述

本示例展示 Agno 使用 **SqliteDb** 作为 **Agent** 会话/运行存储：`Agent(db=db, ...)` 将多轮对话与元数据写入该后端；与具体模型无关，重点在 **存储适配器** 与 `add_history_to_context` 行为。

**核心配置一览：**

| 配置项 | 值 | 说明 |
|--------|------|------|
| `db` | `SqliteDb(db_file=tmp/data.db)` | 本地文件 |
...
`SqliteDb` 实现 `BaseDb` 契约；会话键由 `session_id` / `user_id`（若传入）决定。
