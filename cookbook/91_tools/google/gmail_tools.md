...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'Gmail Reader Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a Gmail reading specialist that can search, read and label emails.' | 来自源码中 `Agent(...)` 字面量或名 |
| `output_schema` | `FindEmailOutput`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Safe Gmail Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a Gmail agent with safe operations only.' | 来自源码中 `Agent(...)` 字面量或名 |
| `output_schema` | `FindEmailOutput`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Gmail Label Manager' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a Gmail label management specialist that helps organize emails with labels.' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Full Gmail Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are an expert Gmail Agent that can read, draft, send and label emails using Gmail.' | 来自源码中 `Agent(...)` 字面量或名 |
| `output_schema` | `FindEmailOutput`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Thread Reply Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a Gmail agent that finds conversations and drafts threaded replies.' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ gmail_tools.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 gmail_tools.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>gmail_tools.py"] --> R["【关键】运行入口<br/>run / main / server"]
