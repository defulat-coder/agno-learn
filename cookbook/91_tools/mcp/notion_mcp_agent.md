...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'NotionDocsAgent' | `Agent(...)` |
| `model` | OpenAIChat(id='gpt-4o'…) | `Agent(...)` |
| `description` | 'Agent to query and modify Notion docs via MCP' | `Agent(...)` |
| `instructions` | dedent('                You have access to Notion documents through MCP tools.\n                - Use tools to read, search, or update pages.\n                - Confirm with the user before making modifications.\n            '…) | `Agent(...)` |
| `markdown` | True | `Agent(...)` |
...
┌──────────────────────┐         ┌────────────────────────────────┐
│ notion_mcp_agent.py  │  ──▶  │ Agent → get_run_messages → Model │
└──────────────────────┘         └────────────────────────────────┘
                                          │
                                          ▼
                                  ┌───────────────┐
...
```text
--- description ---
Agent to query and modify Notion docs via MCP
```
