# notion_mcp_agent.py — 实现原理分析

> 源文件：`cookbook/91_tools/mcp/notion_mcp_agent.py`
...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'NotionDocsAgent' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'Agent to query and modify Notion docs via MCP' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ notion_mcp_agent.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 notion_mcp_agent.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>notion_mcp_agent.py"] --> R["【关键】运行入口<br/>run / main / server"]
