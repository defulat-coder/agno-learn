...
> 源文件：`cookbook/91_tools/mcp/streamable_http_transport/server.py`

## 概述

Start an example MCP server that uses the Streamable HTTP transport.

本示例在 Agno cookbook 中的范式归类：**MCP / 服务脚本**。
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ server.py      │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
4. **定位**：本文件属于 cookbook 演示代码，用于说明 **MCP / 服务脚本** 相关用法。
...
```text
（请对照本目录下 server.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>server.py"] --> R["【关键】运行入口<br/>run / main / server"]
