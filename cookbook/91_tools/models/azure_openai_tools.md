...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'Mixed OpenAI Generator' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'An AI assistant that uses standard OpenAI for chat and Azure OpenAI for image generation' | 来自源码中 `Agent(...)` 字面量或名 |
| `model` | `azure_model`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Full Azure OpenAI Generator' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'An AI assistant that uses Azure OpenAI for both chat and image generation' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ azure_openai_tools.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 azure_openai_tools.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>azure_openai_tools.py"] --> R["【关键】运行入口<br/>run / main / server"]
