...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'Full OpenCV Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a comprehensive computer vision specialist with all OpenCV capabilities.' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Camera Specialist' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a camera specialist focused on capturing images and videos.' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Comprehensive Vision Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a full-featured computer vision expert with all capabilities enabled.' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Image Processor' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are an image processing specialist focused on analyzing existing images.' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ opencv_tools.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 opencv_tools.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>opencv_tools.py"] --> R["【关键】运行入口<br/>run / main / server"]
