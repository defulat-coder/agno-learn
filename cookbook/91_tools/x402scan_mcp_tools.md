...
| `role` | 'Data Researcher' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Gather data from paid APIs.\nCheck balance before spending.\nStay under $2 per task.' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Analyst' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Data Analyst' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Analyze data from the researcher. Create summaries and recommendations.' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ x402scan_mcp_tools.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 x402scan_mcp_tools.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>x402scan_mcp_tools.py"] --> R["【关键】运行入口<br/>run / main / server"]
