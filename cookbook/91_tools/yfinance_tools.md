...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `description` | 'You are a comprehensive investment analyst with access to all financial data functions.' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a basic stock information specialist focused on price and historical data.' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a stock analyst focused on market data without complex financial statements.' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are an equity research analyst focused on recommendations and market sentiment.' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a comprehensive investment analyst with access to all financial data functions.' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ yfinance_tools.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 yfinance_tools.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>yfinance_tools.py"] --> R["【关键】运行入口<br/>run / main / server"]
