# include_exclude_tools_custom_toolkit.py — 实现原理分析

> 源文件：`cookbook/91_tools/other/include_exclude_tools_custom_toolkit.py`
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ include_exclude_tools_custom_toolkit.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 include_exclude_tools_custom_toolkit.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>include_exclude_tools_custom_toolkit.py"] --> R["【关键】运行入口<br/>run / main / server"]
