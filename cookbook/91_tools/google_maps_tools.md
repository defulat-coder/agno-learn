...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'Full Maps API Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a location and business information specialist with full Google Maps access.' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Search-focused Maps Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a location search specialist focused only on finding places.' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Safe Maps API Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `description` | 'You are a location specialist with restricted access to expensive operations.' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ google_maps_tools.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 google_maps_tools.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>google_maps_tools.py"] --> R["【关键】运行入口<br/>run / main / server"]
