...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'Video Caption Generator Agent' | `Agent(...)` |
| `model` | OpenAIChat(id='gpt-4o'…) | `Agent(...)` |
| `description` | 'You are an AI agent that can generate and embed captions for videos.' | `Agent(...)` |
| `markdown` | True | `Agent(...)` |
...
┌──────────────────────┐         ┌────────────────────────────────┐
│ moviepy_video_tools.py │  ──▶  │ Agent → get_run_messages → Model │
└──────────────────────┘         └────────────────────────────────┘
                                          │
                                          ▼
                                  ┌───────────────┐
