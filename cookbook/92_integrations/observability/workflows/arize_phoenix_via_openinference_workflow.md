...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'Researcher' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Research the given topic and provide detailed findings.' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Summarizer' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Create a clear summary of the research findings.' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Fact Checker' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Verify facts and check for accuracy in the research.' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Writer' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Write a comprehensive article based on all available research and verification.' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ arize_phoenix_via_openinference_workflow.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 arize_phoenix_via_openinference_workflow.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>arize_phoenix_via_openinference_workflow.py"] --> R["【关键】运行入口<br/>run / main / server"]
