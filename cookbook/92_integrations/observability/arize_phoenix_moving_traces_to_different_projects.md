...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'Stock Price Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a stock price agent. Answer questions in the style of a stock analyst.' | 来自源码中 `Agent(...)` 字面量或名 |
| `session_id` | 'stock_session' | 来自源码中 `Agent(...)` 字面量或名 |
| `output_schema` | `StockPrice`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Search Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a search agent. Find and summarize information from the web.' | 来自源码中 `Agent(...)` 字面量或名 |
| `session_id` | 'search_session' | 来自源码中 `Agent(...)` 字面量或名 |
| `output_schema` | `SearchResult`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ arize_phoenix_moving_traces_to_different_projects.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 arize_phoenix_moving_traces_to_different_projects.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>arize_phoenix_moving_traces_to_different_projects.py"] --> R["【关键】运行入口<br/>run / main / server"]
