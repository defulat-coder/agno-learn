# mlflow_via_openinference.py — 实现原理分析

> 源文件：`cookbook/92_integrations/observability/mlflow_via_openinference.py`
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ mlflow_via_openinference.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 mlflow_via_openinference.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>mlflow_via_openinference.py"] --> R["【关键】运行入口<br/>run / main / server"]
