...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `add_history_to_context` | `True` | 显式参数 |
| `markdown` | `True` | 显式参数 |
| `model` | `AIMLAPI(id='meta-llama/Llama-3.2-11B-Vision-Instruct-Turbo')` | 显式参数 |
| `num_history_runs` | `3` | 显式参数 |
| `db` | `None` | 未设置 |
| `knowledge` | `None` | 未设置 |
| `tools` | `None` | 未设置 |
...
用户代码层                agno.agent 层
┌──────────────────┐    ┌──────────────────────────────────┐
│ image_agent_with_m│    │ Agent._run / _arun               │
