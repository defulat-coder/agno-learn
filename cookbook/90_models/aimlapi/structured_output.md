...
> 源文件：`cookbook/90_models/aimlapi/structured_output.py`
...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `description` | `'You help people write movie scripts.'` | 显式参数 |
| `model` | `AIMLAPI(id='gpt-5.2')` | 显式参数 |
| `output_schema` | `MovieScript` | 显式参数 |
| `use_json_mode` | `True` | 显式参数 |
| `db` | `None` | 未设置 |
| `knowledge` | `None` | 未设置 |
| `tools` | `None` | 未设置 |
...
用户代码层                agno.agent 层
┌──────────────────┐    ┌──────────────────────────────────┐
│ structured_output.│    │ Agent._run / _arun               │
