...
本示例归类：**单 Agent**；模型相关类型：`Siliconflow`。

**核心配置一览：**

| 配置项 | 值 | 说明 |
|--------|------|------|
| `model` | Siliconflow(id='openai/gpt-oss-120b'…) | `Agent(...)` |
| `description` | 'You help people write movie scripts.' | `Agent(...)` |
| `response_model` | 变量 `MovieScript` | `Agent(...)` |
| `use_json_mode` | True | `Agent(...)` |
| （Model 类） | `Siliconflow` | `agno.models` |
