# code_execution.py — 实现原理分析

> 源文件：`cookbook/90_models/anthropic/code_execution.py`
...
|--------|------|------|
| `markdown` | `True` | 显式参数 |
| `model` | `Claude(id='claude-sonnet-4-20250514', betas=['code-execution-2025-05-22'])` | 显式参数 |
| `tools` | `[{'type': 'code_execution_20250522', 'name': 'code_execution'}]` | 显式参数 |
| `db` | `None` | 未设置 |
| `knowledge` | `None` | 未设置 |
...
用户代码层                agno.agent 层
┌──────────────────┐    ┌──────────────────────────────────┐
│ code_execution.py │    │ Agent._run / _arun               │
...
4. **定位**：位于 `cookbook` 的 **第三方模型适配** 主题，对照同目录示例可看出参数差异。
