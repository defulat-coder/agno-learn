...
> 源文件：`cookbook/90_models/perplexity/memory.py`

## 概述

This recipe shows how to use personalized memories and summaries in an agent.

本示例归类：**单 Agent**；模型相关类型：`Perplexity`。

**核心配置一览：**

| 配置项 | 值 | 说明 |
|--------|------|------|
| `model` | Perplexity(id='sonar-pro'…) | `Agent(...)` |
| `db` | PostgresDb(…) | `Agent(...)` |
| `update_memory_on_run` | True | `Agent(...)` |
| `enable_session_summaries` | True | `Agent(...)` |
| （Model 类） | `Perplexity` | `agno.models` |
...
┌──────────────────────┐         ┌────────────────────────────────┐
│ memory.py            │  ──▶  │ Agent → get_run_messages → Model │
└──────────────────────┘         └────────────────────────────────┘
                                          │
                                          ▼
                                  ┌───────────────┐
