...
本文件为 cookbook 示例 `cookbook_runner.py`，范式概览：**脚本或非典型 Agent 入口**。

本示例在 Agno cookbook 中的范式归类：**脚本或非典型 Agent 入口**。
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ cookbook_runner.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
4. **定位**：本文件属于 cookbook 演示代码，用于说明 **脚本或非典型 Agent 入口** 相关用法。
...
```text
（请对照本目录下 cookbook_runner.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>cookbook_runner.py"] --> R["【关键】运行入口<br/>run / main / server"]
