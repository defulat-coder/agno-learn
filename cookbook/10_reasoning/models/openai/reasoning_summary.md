...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `instructions` | `'Use tables to display the analysis'` | 显式参数 |
| `markdown` | `True` | 显式参数 |
| `model` | `OpenAIResponses(id='o4-mini', reasoning_summary='auto')` | 显式参数 |
| `tools` | `[WebSearchTools(enable_news=False)]` | 显式参数 |
| `db` | `None` | 未设置 |
| `knowledge` | `None` | 未设置 |
...
用户代码层                agno.agent 层
┌──────────────────┐    ┌──────────────────────────────────┐
│ reasoning_summary.│    │ Agent._run / _arun               │
...
| 1 | `description` | 未设置 | 否 |
| 2 | `role` | 未设置 | 否 |
| 3 | `instructions` | 'Use tables to display the analysis' | 是 |
| 4.1 | `markdown` | True | 是 |
...
```text
Use tables to display the analysis
