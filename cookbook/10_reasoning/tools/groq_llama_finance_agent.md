...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `instructions` | `dedent('        ## General Instructions\n        - Always start by using the think tool to map out the steps needed to complete the task.\n        - After receiving tool results, u...` | 显式参数 |
| `markdown` | `True` | 显式参数 |
| `model` | `Groq(id='meta-llama/llama-4-scout-17b-16e-instruct')` | 显式参数 |
| `tools` | `[ReasoningTools(), WebSearchTools()]` | 显式参数 |
| `db` | `None` | 未设置 |
| `knowledge` | `None` | 未设置 |
...
用户代码层                agno.agent 层
┌──────────────────┐    ┌──────────────────────────────────┐
│ groq_llama_finance│    │ Agent._run / _arun               │
...
| 1 | `description` | 未设置 | 否 |
| 2 | `role` | 未设置 | 否 |
| 3 | `instructions` | `dedent('        ## General Instructions\n        - Always start by using the think tool to map out the steps needed to c...` | 是 |
| 4.1 | `markdown` | True | 是 |
