...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `instructions` | `'Use tables where possible'` | 显式参数 |
| `markdown` | `True` | 显式参数 |
| `model` | `Llama(id='Llama-4-Maverick-17B-128E-Instruct-FP8')` | 显式参数 |
| `tools` | `[ReasoningTools(enable_think=True, enable_analyze=True, add_instructions=True), YFinanceTools()]` | 显式参数 |
| `db` | `None` | 未设置 |
| `knowledge` | `None` | 未设置 |
...
用户代码层                agno.agent 层
┌──────────────────┐    ┌──────────────────────────────────┐
│ llama_reasoning_to│    │ Agent._run / _arun               │
...
| 1 | `description` | 未设置 | 否 |
| 2 | `role` | 未设置 | 否 |
| 3 | `instructions` | 'Use tables where possible' | 是 |
| 4.1 | `markdown` | True | 是 |
