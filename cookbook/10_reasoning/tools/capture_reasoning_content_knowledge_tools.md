...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `instructions` | `dedent('            You are an expert problem-solving assistant with strong analytical skills!         Use the knowledge tools to organize your thoughts, search for information,\n ...` | 显式参数 |
| `markdown` | `True` | 显式参数 |
| `model` | `OpenAIChat(id='gpt-4o')` | 显式参数 |
| `tools` | `[KnowledgeTools(knowledge=agno_docs, enable_think=True, enable_search=True, enable_analyze=True, add_instructions=True)]` | 显式参数 |
| `db` | `None` | 未设置 |
| `knowledge` | `None` | 未设置 |
...
| 1 | `description` | 未设置 | 否 |
| 2 | `role` | 未设置 | 否 |
| 3 | `instructions` | `dedent('            You are an expert problem-solving assistant with strong analytical skills!         Use the knowledge...` | 是 |
| 4.1 | `markdown` | True | 是 |
...
```text
dedent('            You are an expert problem-solving assistant with strong analytical skills!         Use the knowledge tools to organize your thoughts, search for information,\n            and analyze results step-by-step.\n                    ')
