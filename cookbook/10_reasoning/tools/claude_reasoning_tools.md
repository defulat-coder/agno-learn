...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `instructions` | `'Use tables to display data.'` | 显式参数 |
| `markdown` | `True` | 显式参数 |
| `model` | `Claude(id='claude-sonnet-4-20250514')` | 显式参数 |
| `tools` | `[ReasoningTools(add_instructions=True), WebSearchTools(enable_news=False)]` | 显式参数 |
| `db` | `None` | 未设置 |
| `knowledge` | `None` | 未设置 |
...
用户代码层                agno.agent 层
┌──────────────────┐    ┌──────────────────────────────────┐
│ claude_reasoning_t│    │ Agent._run / _arun               │
