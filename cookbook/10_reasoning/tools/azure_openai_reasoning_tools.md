# azure_openai_reasoning_tools.py — 实现原理分析
...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `instructions` | `'Use tables where possible. Think about the problem step by step.'` | 显式参数 |
| `markdown` | `True` | 显式参数 |
| `model` | `AzureOpenAI(id='gpt-4o-mini')` | 显式参数 |
| `tools` | `[WebSearchTools(), ReasoningTools(enable_think=True, enable_analyze=True, add_instructions=True, add_few_shot=True)]` | 显式参数 |
| `db` | `None` | 未设置 |
| `knowledge` | `None` | 未设置 |
...
用户代码层                agno.agent 层
┌──────────────────┐    ┌──────────────────────────────────┐
│ azure_openai_reaso│    │ Agent._run / _arun               │
...
| 1 | `description` | 未设置 | 否 |
| 2 | `role` | 未设置 | 否 |
| 3 | `instructions` | 'Use tables where possible. Think about the problem step by step.' | 是 |
| 4.1 | `markdown` | True | 是 |
...
```text
Use tables where possible. Think about the problem step by step.
