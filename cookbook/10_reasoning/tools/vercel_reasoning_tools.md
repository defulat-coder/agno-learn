...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `instructions` | `['Use tables to display data', 'Only output the report, no other text']` | 显式参数 |
| `markdown` | `True` | 显式参数 |
| `model` | `V0(id='v0-1.0-md')` | 显式参数 |
| `tools` | `[ReasoningTools(add_instructions=True, add_few_shot=True), WebSearchTools()]` | 显式参数 |
| `db` | `None` | 未设置 |
| `knowledge` | `None` | 未设置 |
...
用户代码层                agno.agent 层
┌──────────────────┐    ┌──────────────────────────────────┐
│ vercel_reasoning_t│    │ Agent._run / _arun               │
...
| 1 | `description` | 未设置 | 否 |
| 2 | `role` | 未设置 | 否 |
| 3 | `instructions` | ['Use tables to display data', 'Only output the report, no other text'] | 是 |
| 4.1 | `markdown` | True | 是 |
...
```text
['Use tables to display data', 'Only output the report, no other text']
