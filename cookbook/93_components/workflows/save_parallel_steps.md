...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'HackerNews Researcher' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Research tech news and trends from Hacker News' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Web Researcher' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Research general information from the web' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Content Writer' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Write well-structured content from research findings' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Content Reviewer' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Review and improve the written content' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ save_parallel_steps.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 save_parallel_steps.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>save_parallel_steps.py"] --> R["【关键】运行入口<br/>run / main / server"]
