...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'HackerNews Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Research tech news and trends from Hacker News' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Web Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Research general information from the web' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Summary Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'Summarize the research findings into a concise report' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ save_router_steps.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 save_router_steps.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>save_router_steps.py"] --> R["【关键】运行入口<br/>run / main / server"]
