...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `id` | 'hackernews-agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Hackernews Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Extract key insights and content from Hackernews posts' | 来自源码中 `Agent(...)` 字面量或名 |
| `id` | 'web-agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Web Agent' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Search the web for the latest news and trends' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ save_workflow.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 save_workflow.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>save_workflow.py"] --> R["【关键】运行入口<br/>run / main / server"]
