...
| `model` | Gemini(id='gemini-3.1-pro-preview'…) | `Agent(...)` |
| `instructions` | 'You are a professional movie critic. Provide balanced, thoughtful reviews.' | `Agent(...)` |
| `output_schema` | 变量 `MovieReview` | `Agent(...)` |
...
┌──────────────────────┐         ┌────────────────────────────────┐
│ 3_structured_output.py │  ──▶  │ Agent → get_run_messages → Model │
└──────────────────────┘         └────────────────────────────────┘
                                          │
                                          ▼
                                  ┌───────────────┐
...
```text
--- instructions ---
You are a professional movie critic. Provide balanced, thoughtful reviews.
```
