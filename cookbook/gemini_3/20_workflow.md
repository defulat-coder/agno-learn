...
本示例在 Agno cookbook 中的范式归类：**Workflow 为主**。

**核心配置一览：**

| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'Web Researcher' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a web researcher. Search for the latest information on the given topic.\n\n## Rules\n\n- Find recent, credible so...' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Deep Researcher' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a deep researcher. Search extensively for background context,\nhistorical data, and expert analysis on the giv...' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Analyst' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a senior analyst. Synthesize research from multiple sources\ninto a clear, structured analysis.\n\n## Rules\n\n- I...' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Report Writer' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a report writer. Transform analysis into a polished,\npublication-ready report.\n\n## Rules\n\n- Write a compellin...' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Fact Checker' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a fact-checker. Verify the factual claims in the report.\n\n## Rules\n\n- Check every statistic, date, and named ...' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ 20_workflow.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
4. **定位**：本文件属于 cookbook 演示代码，用于说明 **Workflow 为主** 相关用法。
...
```text
（请对照本目录下 20_workflow.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>20_workflow.py"] --> R["【关键】运行入口<br/>run / main / server"]
