...
| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'Video Analyst' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Analyze video clips for visual content, pacing, and mood' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a film analysis expert. Watch video clips and provide detailed breakdowns.\n\n## Describe\n\n- Shot types (wide, ...' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Script Reader' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Read film scripts and extract relevant dialogue and directions' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a script supervisor. Read scripts and extract relevant information.\n\n## Extract\n\n- Scene headings (INT/EXT, l...' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Continuity Editor' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Check consistency between script and footage' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a continuity editor. Compare the script to the footage and flag issues.\n\n## Check\n\n- Does the footage match t...' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ film_scene_breakdown.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 film_scene_breakdown.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>film_scene_breakdown.py"] --> R["【关键】运行入口<br/>run / main / server"]
