...
本示例在 Agno cookbook 中的范式归类：**Team 多智能体**。

**核心配置一览：**

| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'Concept Artist' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Pitch Writer' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Write structured game concept pitches' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a game design consultant. Create compelling, structured game pitches.\n\n## Rules\n\n- Be specific about mechanic...' | 来自源码中 `Agent(...)` 字面量或名 |
| `output_schema` | `GamePitch`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Market Analyst' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Evaluate market viability and competitive landscape' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You analyze game market trends. Evaluate pitches for market viability.\n\n## Evaluate\n\n- Is there market demand for thi...' | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Creative Director' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Evaluate creative vision and player experience' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You evaluate game concepts for creative quality and player appeal.\n\n## Evaluate\n\n- Is the core mechanic fun and origi...' | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ game_concept_pitch.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
4. **定位**：本文件属于 cookbook 演示代码，用于说明 **Team 多智能体** 相关用法。
...
```text
（请对照本目录下 game_concept_pitch.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>game_concept_pitch.py"] --> R["【关键】运行入口<br/>run / main / server"]
