...
| `role` | 'Write engaging blog post drafts' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | `writer_instructions`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `db` | `gemini_agents_db`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Editor' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Review and improve content for clarity and quality' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | `editor_instructions`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `db` | `gemini_agents_db`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'Fact Checker' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Verify factual claims using web search' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | `fact_checker_instructions`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `db` | `gemini_agents_db`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ 19_team.py     │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
```text
（请对照本目录下 19_team.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>19_team.py"] --> R["【关键】运行入口<br/>run / main / server"]
