...
本示例在 Agno cookbook 中的范式归类：**混合 Workflow / Team / Agent**。

**核心配置一览：**

| 配置项 | 值 | 说明 |
|--------|------|------|
| `name` | 'L4 Coder' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Write code based on requirements' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a senior developer. Write clean, well-documented code.\n\n## Workflow\n1. Understand the requirements\n2. Write t...' | 来自源码中 `Agent(...)` 字面量或名 |
| `db` | `db`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'L4 Reviewer' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Review code for quality, bugs, and best practices' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a senior code reviewer. Provide thorough, constructive reviews.\n\n## Workflow\n1. Read the code files\n2. Check ...' | 来自源码中 `Agent(...)` 字面量或名 |
| `db` | `db`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
| `name` | 'L4 Tester' | 来自源码中 `Agent(...)` 字面量或名 |
| `role` | 'Write and run tests for the code' | 来自源码中 `Agent(...)` 字面量或名 |
| `instructions` | 'You are a QA engineer. Write thorough tests and run them.\n\n## Workflow\n1. Read the implementation code\n2. Write tests...' | 来自源码中 `Agent(...)` 字面量或名 |
| `db` | `db`（变量） | 来自源码中 `Agent(...)` 字面量或名 |
...
用户/脚本层              Agno 框架层
┌────────────────┐      ┌─────────────────────────────────┐
│ level_4_team.py │ ──▶ │ get_system_message / run loop     │
│ （本文件）      │      │ libs/agno/agno/...               │
└────────────────┘      └─────────────────────────────────┘
                                │
                                ▼
...
4. **定位**：本文件属于 cookbook 演示代码，用于说明 **混合 Workflow / Team / Agent** 相关用法。
...
```text
（请对照本目录下 level_4_team.py 中 `Agent` 的字符串字面量自行核对；若为变量拼接或运行时注入，需运行后打印 `get_system_message` 返回值。）
```
...
```mermaid
flowchart TD
    U["用户/入口<br/>level_4_team.py"] --> R["【关键】运行入口<br/>run / main / server"]
