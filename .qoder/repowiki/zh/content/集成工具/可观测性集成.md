...
- 与 Langfuse、Arize Phoenix、LangSmith、OpenTelemetry 等平台的集成配置与使用
...
I["workflows/arize_phoenix_via_openinference_workflow.md"]
end
subgraph "追踪核心"
J["tracing/__init__.py"]
K["tracing/schemas.py"]
L["tracing/setup.py"]
M["tracing/exporter.py"]
end
subgraph "日志"
N["utils/log.py"]
O["utilities/logging.py"]
P["tests/unit/utils/test_logging.py"]
Q["cookbook/02_agents/14_advanced/custom_logging.md"]
end
subgraph "指标与评估"
R["os/routers/metrics/metrics.py"]
S["tests/system/tests/test_metrics_routes.py"]
T["eval/performance.py"]
U["cookbook/09_evals/performance/async_function.py.md"]
end
A --> B
A --> C
A --> D
...
- [__init__.py:1-13](file://libs/agno/agno/tracing/__init__.py#L1-L13)
...
## 架构总览
下图展示了从应用到追踪后端的整体链路，包括本地数据库导出与云平台导出两种路径。
...
- 导出器选择：数据库导出适合离线分析与审计；云平台导出适合实时可视化与协作。
...
## 结论
本项目提供了从本地数据库到主流云平台的完整可观测性方案，涵盖追踪、日志与性能评估。通过统一的 Tracing API 与灵活的导出策略，可在不同场景下快速落地可观测性能力，并结合平台特性实现高效的问题定位与性能优化。
...
**章节来源**
- [README.md:30-35](file://cookbook/92_integrations/observability/README.md#L30-L35)