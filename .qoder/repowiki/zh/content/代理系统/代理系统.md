...
## 更新摘要
**变更内容**
- 新增内置 followup suggestions 系统，提供基于对话上下文的智能后续提示
- 支持所有运行路径：同步、异步、流式和异步流式
- 自动生成结构化后续问题列表，提升用户体验
- 包含事件流支持和指标统计
...
**更新** 新增内置 followup suggestions 系统，为用户提供智能后续提示功能，支持所有运行路径。
...
- Followups 系统：内置智能后续提示生成器，基于对话上下文自动生成结构化后续问题。
- 多模态与媒体：支持图片、音频、视频、文件的收集与传递，可选择是否发送给模型或仅用于工具。

**更新** 新增 Followups 系统，提供智能后续提示功能。
...
## 架构总览
下图展示一次典型运行从 Agent 入口到工具执行与结果回填的消息流与职责划分，包括新增的 Followups 系统。
...
PostProc --> Followups["生成后续提示"]
Followups --> PostHooks["执行后钩子"]
...
### 10) 内置后续提示系统 (新增)
- 功能概述
  - followups：启用内置后续提示功能
  - num_followups：控制生成的后续提示数量（默认3个）
  - followup_model：可选的专用模型用于生成后续提示
  - followups 属性：存储生成的结构化后续问题列表
- 实现机制
  - 同步模式：generate_followups 在主响应完成后生成后续提示
  - 异步模式：agenerate_followups 异步生成后续提示
  - 流式模式：generate_followups_stream 和 agenerate_followups_stream 支持事件流
  - 消息构建：_build_followup_messages 基于对话上下文构建提示消息
  - 响应解析：_parse_followups_response 解析模型输出为结构化列表
- 事件支持
  - create_followups_started_event：开始生成事件
  - create_followups_completed_event：完成生成事件
  - 支持流式事件传输和存储
- 指标统计
  - _accumulate_followups_metrics 累计后续提示生成的模型使用指标
  - 使用 ModelType.FOLLOWUP_MODEL 标识后续提示模型

```mermaid
flowchart TD
Start(["生成后续提示"]) --> Check{"followups启用?"}
Check -- 否 --> End1["跳过生成"]
Check -- 是 --> ModelSel["选择模型(followup_model或agent.model)"]
ModelSel --> MsgBuild["构建提示消息(基于对话上下文)"]
MsgBuild --> CallModel["调用模型生成提示"]
CallModel --> ParseResp["解析响应为结构化列表"]
ParseResp --> Metrics["累计模型使用指标"]
Metrics --> Store["存储到RunOutput.followups"]
Store --> Events["生成完成事件"]
Events --> End2["返回结果"]
```

**图表来源**
- [_response.py:1780-1919](file://libs/agno/agno/agent/_response.py#L1780-L1919)
- [_run.py:566-567](file://libs/agno/agno/agent/_run.py#L566-L567)
- [_run.py:972-977](file://libs/agno/agno/agent/_run.py#L972-L977)
- [_run.py:1650-1651](file://libs/agno/agno/agent/_run.py#L1650-L1651)
- [_run.py:2167-2173](file://libs/agno/agno/agent/_run.py#L2167-L2173)

**章节来源**
- [agent.py:305-312](file://libs/agno/agno/agent/agent.py#L305-L312)
- [_response.py:1780-1919](file://libs/agno/agno/agent/_response.py#L1780-L1919)
- [_run.py:566-567](file://libs/agno/agno/agent/_run.py#L566-L567)
- [_run.py:972-977](file://libs/agno/agno/agent/_run.py#L972-L977)
- [_run.py:1650-1651](file://libs/agno/agno/agent/_run.py#L1650-L1651)
...
- 组件耦合
  ...
  - Followups 系统与 Agent 主系统紧密集成，在运行循环的适当节点调用。
- 外部依赖
...
- 异步与并发
  ...
  - Followups 系统支持异步生成，不影响主响应流程。
- 上下文压缩
  - compress_tool_results 与 CompressionManager 减少上下文长度。
- 指标与可观测
  - 会话指标累加与事件存储，便于定位性能瓶颈。
  - Followups 模型使用指标独立统计。

**更新** Followups 系统提供独立的模型使用指标统计，便于监控额外的计算开销。
...
- Followups 生成失败
  - 检查 followups 配置、模型可用性和网络连接。
  - 确认 followup_model 设置正确或使用 agent.model。
  - 查看日志中的错误信息以诊断具体问题。

**更新** 新增 Followups 相关故障排查指导。
...
**更新** 新增的内置 Followups 系统进一步提升了用户体验，通过智能后续提示增强对话的连贯性和交互效率，支持所有运行路径且具备完善的事件流和指标统计能力。
...
- 最佳实践
  ...
  - 多模态场景下谨慎控制媒体传递与存储成本。
  - Followups 配置建议：根据业务需求调整 num_followups 数量，考虑使用更便宜的模型专门处理后续提示生成。
  - 监控 Followups 模型使用指标，评估额外的计算成本和响应时间影响。