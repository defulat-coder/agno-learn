...
- [libs/agno/tests/integration/teams/test_followups.py](file://libs/agno/tests/integration/teams/test_followups.py)
- [cookbook/02_agents/02_input_output/followup_suggestions.py](file://cookbook/02_agents/02_input_output/followup_suggestions.py)
- [cookbook/02_agents/02_input_output/followup_suggestions_streaming.py](file://cookbook/02_agents/02_input_output/followup_suggestions_streaming.py)
</cite>

## 更新摘要
**所做更改**
- 新增团队后续提示系统章节，详细介绍 Team 类内置的 followup suggestions 功能
- 更新团队模式与编排章节，增加后续提示功能的集成说明
- 新增后续提示事件系统章节，说明 followups_started 和 followups_completed 事件
- 更新架构总览图，加入后续提示生成流程
- 新增后续提示最佳实践和使用指南
...
**更新** 新增团队后续提示系统，支持基于对话上下文的智能后续建议生成功能，可在所有运行路径中使用。
...
- **后续提示系统**
  - Team 类内置 followup suggestions 功能，自动生成基于对话上下文的智能后续建议。
  - 支持同步、异步、流式和事件驱动的后续提示生成。
  - 可配置数量、模型和响应格式，兼容所有运行模式。
- 高级能力
...
## 架构总览
下图展示一个典型"路由模式"的团队运行时交互，包含后续提示生成流程：客户端请求进入 Team，Team 根据模式与成员配置进行路由，成员执行工具调用与推理，生成主响应，随后 Team 自动生成后续提示建议，最终返回结构化或流式结果，并可触发后置钩子。
...
participant Followups as "后续提示系统"
...
Tools-->>Member : "工具结果"
Member-->>Team : "主响应"
Team->>Followups : "生成后续提示建议"
Followups-->>Team : "后续提示列表"
Team-->>Client : "主响应+后续提示"
...
- [libs/agno/agno/team/_response.py:1755-1787](file://libs/agno/agno/team/_response.py#L1755-L1787)
- [libs/agno/agno/utils/events.py:981-1002](file://libs/agno/agno/utils/events.py#L981-L1002)
...
- 功能概述
  ...
  - 可配置数量、模型和响应格式，兼容所有运行模式。
- 核心配置参数
  - followups: bool = False - 启用/禁用后续提示功能
  - num_followups: int = 3 - 生成的后续提示数量（默认3个）
  - followup_model: Optional[Model] = None - 用于生成后续提示的专用模型
- 事件系统
  - TeamFollowupsStartedEvent: 后续提示生成开始事件
  - TeamFollowupsCompletedEvent: 后续提示生成完成事件
- 数据结构
  - RunOutput.followups: Optional[List[str]] - 后续提示列表
  - TeamRunOutput.followups: Optional[List[str]] - 团队运行后的后续提示列表

```mermaid
flowchart TD
A["团队运行完成"] --> B{"followups启用？"}
B --> |否| C["直接返回结果"]
B --> |是| D["检查followup_model或使用team.model"]
D --> E["构建后续提示消息"]
E --> F["调用模型生成建议"]
F --> G["解析响应为结构化列表"]
G --> H["触发followups_completed事件"]
H --> I["返回包含后续提示的结果"]
```
...
- [libs/agno/agno/team/_response.py:1755-1787](file://libs/agno/agno/team/_response.py#L1755-L1787)
- [libs/agno/agno/utils/events.py:981-1002](file://libs/agno/agno/utils/events.py#L981-L1002)
- [libs/agno/agno/os/schema.py:384-449](file://libs/agno/agno/os/schema.py#L384-L449)

章节来源
- [libs/agno/agno/team/team.py:335-341](file://libs/agno/agno/team/team.py#L335-L341)
- [libs/agno/agno/team/_response.py:1755-1787](file://libs/agno/agno/team/_response.py#L1755-L1787)
- [libs/agno/agno/utils/events.py:981-1002](file://libs/agno/agno/utils/events.py#L981-L1002)
- [libs/agno/agno/os/schema.py:384-449](file://libs/agno/agno/os/schema.py#L384-L449)
- [libs/agno/tests/integration/teams/test_followups.py:1-96](file://libs/agno/tests/integration/teams/test_followups.py#L1-L96)

## 依赖分析
- 组件耦合
  - Team 与 Agent、MemoryManager、工具、钩子、存储之间存在清晰边界；通过接口解耦，便于替换与扩展。
  - 后续提示系统与 Team 主运行流程解耦，通过事件机制集成。
- 外部依赖
  - 向量数据库（PgVector/LanceDB）、外部 API（如 yfinance）、数据库（Postgres）等。
  - 后续提示系统依赖模型响应格式和事件处理机制。
- 循环依赖
...
Team --> Followups["后续提示系统"]
Followups --> Model["模型响应格式"]
Followups --> Events["事件处理"]
```
...
- **后续提示性能优化**
  - 后续提示生成作为独立的模型调用，可配置专用的低成本模型。
  - 支持异步生成，避免阻塞主响应流程。
  - 事件驱动的后续提示收集，减少不必要的等待时间。
...
- **后续提示问题排查**
  - 后续提示未生成：检查 followups=True 配置，确认模型可用性。
  - 后续提示格式错误：验证响应格式配置，检查模型是否支持结构化输出。
  - 事件未触发：确认 stream_events=True 设置，检查事件过滤配置。
...
## 结论
Agno Learn 的团队系统提供了从基础编排到高级能力的完整能力矩阵：通过路由/广播/任务/协调四种模式满足多样化协作需求；借助工具钩子与权限控制实现灵活扩展；通过结构化输入输出与流式处理优化交互体验；通过知识库、分布式检索与搜索协调强化信息获取；通过内存与学习实现长期记忆与个性化；通过护栏与合规保障安全；通过多模态与人机协作拓展应用场景；通过会话状态与指标监控实现可观测性与可维护性；**通过内置后续提示系统，为用户提供智能化的对话引导和交互增强**。建议在实际落地中优先从路由/广播模式入手，逐步引入上下文压缩、分布式 RAG、自学习、护栏和后续提示等能力，以渐进方式构建稳健的多代理团队系统。
...
- 最佳实践清单
  ...
  - **后续提示优化**：根据业务场景调整提示数量和模型配置，平衡质量和成本；在流式场景中合理使用事件机制收集后续提示。